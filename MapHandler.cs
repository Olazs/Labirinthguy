...
namespace LabrinthGuy
{
    class MapHandler
    {
       ...
       public void stats(Player p)
        {
            this.message = $"Eletero:{p.hp} Szint:{p.lvl} Ero:{p.strength} Ugyesseg:{p.agility} Kitartas:{p.endurance} Fegyver:{p.weapon}";
        }
      
    }
    class weaponswitch : Enemy
    {
        public void weapon_switch(Player p, Enemy e)
        {
            Weapon enemy_weapon = e.weapon;
            p.weapon = enemy_weapon;
        }
    }
}
